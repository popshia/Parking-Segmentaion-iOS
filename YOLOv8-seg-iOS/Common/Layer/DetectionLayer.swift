...
class DetectionLayer: CALayer {
    ...
    func addDetection(
        ...
        let shapeLayer = createRoundedRectLayerWithBounds(objectBounds, showBox: showBox)
    ...
    private func createAnnotationLayer(
        ...
        let text = String(format: "%@ %.2f", identifier, confidence)
    ...
    private func createRoundedRectLayerWithBounds(_ bounds: CGRect, showBox: Bool) -> CALayer {
        let shapeLayer = CALayer()
        shapeLayer.borderColor = UIColor.yellow.cgColor
        shapeLayer.borderWidth = showBox ? 2 : 0
